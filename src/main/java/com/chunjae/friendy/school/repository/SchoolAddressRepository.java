...
public interface SchoolAddressRepository extends JpaRepository<SchoolAddress, Long> {
    ...
    @Query(value = "SELECT sa FROM school_address sa  WHERE sa.roadAddress LIKE %:searchCity% or sa.roadAddressDetail LIKE %:searchCity%")
    Page<SchoolAddress> findBySearchCityUsingJoin(@Param("searchCity") String searchCity, Pageable pageable);

    @Query(value = "SELECT sa FROM school_address sa WHERE sa.roadAddress LIKE %:searchKeyword% or sa.roadAddressDetail like %:searchKeyword%")
    ...
    @Query(value = "SELECT sa FROM school_address sa inner join school s on s.idx = sa.school_idx WHERE sa.roadAddress LIKE %:district% or sa.roadAddressDetail LIKE %:district%")
    Page<SchoolAddress> findByDistrictUsingJoin(@Param("district")String district, Pageable pageable);

    @Query(value = "SELECT sa FROM school s join school_address sa on s.idx = sa.school_idx WHERE  s.district LIKE %:district% and (sa.roadAddress like %:searchKeyword% or sa.roadAddressDetail like %:searchKeyword%)")
